...
**Gates humanos fora de yolo:** liberação 001, aprovação 003, `critical` em 005, item `(user)`, bloqueio e merge. Em yolo, crítico strong assume 003/005; o humano só reaparece no circuit breaker/item user/merge final.

**Task `yolo: true`:** 003/005 usam sessões strong limpas da [[.agents/skills/yolo-critic/SKILL|yolo-critic]]. Cada gate permite duas devoluções automáticas; 3ª falha ativa circuit breaker. Execute waves de até três tasks sem dependência, overlap de escrita ou repo não isolado. 006 é mecânico/idempotente: meta PoP permanece em `main`; externo integra tasks em `develop` e abre PR final `develop` → `main`, sem merge do agente.
...
- **005:** revisor independente; em yolo sempre strong e sessão limpa. Ele escolhe `differential|full`, registra motivo/superfície/testes e usa full em critical ou após retorno.
- **003 yolo:** crítico strong assina ou devolve com motivos objetivos. Em 003 e 005, devoluções 1–2 retornam automaticamente; 3ª falha ativa `circuit_breaker`.

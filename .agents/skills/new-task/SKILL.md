...
**Entrada por pedido de alteração — triagem primeiro:** se o humano pedir para aplicar, executar, corrigir ou finalizar algo e não houver card ativo, aplique a **triagem da regra 13** do [[AGENTS|AGENTS]] antes de criar task: escopo evidente pelo próprio pedido + nenhum contrato durável novo + sem entrevista de planejamento + cabe numa sessão → é **fix direto** (seção "Fix direto" do [[WORKFLOW|WORKFLOW]]: sem card, prova em memory `F-` + specs) e esta skill não roda. Qualquer "não" → task por aqui e depois `advance-task`; na dúvida, uma pergunta de uma linha. “Iniciar o fluxo em yolo” define `yolo: true`, confirma a materialização/liberação e encadeia a rota yolo integral. Reuse tudo que a conversa já respondeu e pergunte só uma ambiguidade indispensável de projeto/origem; waiver do kanban exige dispensa literal e segue o protocolo do WORKFLOW.

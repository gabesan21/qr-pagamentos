...
**Você apenas decide se algo precisa ser ajustado.** Julga por **leitura** — diff integrado e evidência registrada — e **nunca re-roda critérios nem executa testes**: teste pertence à task de verificação da phase ([[WORKFLOW|WORKFLOW]] › "Verificação de phase"). A única exceção é a própria task `verificacao-da-phase`, cujo plano declara re-run como critério `agent`.
...
3. Escolha `differential` ou `full` e registre motivo/superfície. `full` para `critical: true` e retorno por `premissa`; depois de `lacuna`/execução, o diferencial cobre o **delta** e audita o resto por evidência (`pop/scripts/pop_yolo.py verify-mode <id>` calcula).
...
6. **Separe quem falhou — e o tamanho do defeito.** Bloqueante **pontual** (`arquivo:linha`, remédio objetivo, sem mudança de estratégia) pede **reparo dirigido**: registre o delta, o orquestrador despacha o patch e você o confere em adendo ≤10 linhas nesta rodada — não é rota nem consome contador (máx. 2 por rodada; o 3º vira rota). Para o resto, três saídas: executor não cumpriu o que recebeu → **004** (`execucao`); critérios não cobriam o pedido → **002** (`lacuna` se só falta acrescentar; `premissa` se a estratégia estava errada).
7. **Preencha o `## Delta da devolução`** em toda devolução: tipo, critérios afetados, frentes que reentram e **frentes intactas**. Sem delta, 002 replaneja às cegas e 004 refaz trabalho aprovado.
8. **O gate não expande o escopo:** achado real fora do pedido vira follow-up rastreável, nunca critério novo; `lacuna` cabe uma vez por task.
9. **Aprovando, escreva a memory nesta mesma sessão** — ledger `memory/<AAAA-MM-DD>/<id>.md` mais uma entrada `<id>.<nn>-<slug>.md` por coisa feita, com evidência linkada ([[_templates/MEMORY|MEMORY]] ≤1200 chars · [[_templates/MEMORY-ENTRY|MEMORY-ENTRY]] ≤800). Só a memory — integração, PR, sync de specs e limpeza são do orquestrador.

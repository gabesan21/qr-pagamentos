...
4. **Recon da epoch:** antes de detalhar, pesquise o que embasa o capítulo — **subagentes paralelos, um por frente** — e consolide em `researches/<assunto>/`. O que a pesquisa não resolver vira **RECON NEEDED** no arquivo da epoch, com o check exato que resolve (pesquisa, experimento ou task). Pesquisa que exige profundidade além do alcance do agente → proponha o prompt no **`RESEARCHES.md`** do projeto (opcional, `_templates/RESEARCHES.md`) para o usuário rodar.
...
6. **Tasks candidatas:** para a primeira phase, proponha 2–5 tasks de uma linha. Não crie as pastas no kanban — isso é a skill `new-task`.
...
8. **Materialize:** `ROADMAP.md` (só epochs) e `roadmap/<n>-<slug>.md` da epoch detalhada, pelos templates `_templates/ROADMAP.md` e `_templates/EPOCH.md`. Confirme o resultado com o usuário.

...
const artifactRoot = join(root, "artifacts", "login");
...
if (!review.includes(manifest.runId) || !review.includes(sha256(manifestBytes)) || /unresolved severity [2-4]/i.test(review)) throw new Error("Login review is not bound to the current manifest or still has a severity 2+ finding.");
console.log(`Verified login evidence ${manifest.runId}`);

...
> Types explicados em [[TYPES|TYPES]]: `default` (trabalho em `project/`) | `included` (harness na raiz do repo) | `multi-repo` (vários repos em `project/`) | `full-multi-repo` (vários repos, cada um com harness `included` embutido; kanban central só para tasks cross-repo).

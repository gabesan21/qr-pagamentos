...
> Types explicados em [[TYPES|TYPES]]: specs de IA podem ser commitadas no repo → `included`; repo deve ficar limpo de harness → `default` (clone em `project/`); um de vários repos do mesmo projeto → `multi-repo`; vários repos que devem funcionar standalone, cada um com harness commitado → `full-multi-repo`.
...
- **Decisões históricas** que explicam o desenho atual (viram `notes/decisions/`):

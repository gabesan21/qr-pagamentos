...
2. O usuário roda o prompt onde quiser e entrega o resultado bruto em `researches/<assunto>/raw/` (dica: o Obsidian Web Clipper converte artigos web em markdown).
3. Resultado entregue → o agente roda a skill `ingest-research`: síntese em `researches/<assunto>/<assunto>.md`, status `entregue` com link para a síntese e proposta de updates de roadmap/spec (contradição com spec/nota fica sinalizada, nunca silenciosa).

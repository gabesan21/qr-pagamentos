...
- Validate operator-selected callback URLs before provider dispatch; require absolute HTTPS — or HTTP on a loopback host (`localhost`, `127.0.0.1`, `[::1]`) for local testing — without embedded credentials or fragments. In production (`NODE_ENV=production`), that loopback exception additionally requires `ALLOW_LOOPBACK_OPERATOR_ORIGINS=1` (installer-derived, never hand-set); the decision lives once in `src/net/local-origin.ts`, never restated in this subtree.
...
- Every quote, onramp order creation, order read, and main-wallet balance failure calls the shared `src/observability/provider-failure-log.ts` writer instead of formatting its own record. The record is closed to operation, HTTP status (or a transport-failure marker), and a `code`: a `code` is logged verbatim only when the caller already validated it against a closed documented allowlist (today, the onramp creation refusal codes below and the `POST /pricing/panel/buy` quote-diagnosis codes in the pricing boundary section); every other case — no established allowlist, absent, unparseable, undocumented — omits `code` and the writer resolves the fixed unknown marker. This never re-reads a response body already consumed for classification, and it never weakens the "never expose raw envelopes... or provider error bodies" rule above: the log never carries the body, headers, URL, API key, PIX payload, or customer data.
...
- A non-200 `POST /pricing/panel/buy` response with a parseable body and a `code` in the closed documented allowlist (`validation.exchange_currency_invalid`, `validation.currency_not_found`, `validation.exchange_currency_not_found`, `validation.failed`) is a typed `NauttPricingRefusedError`, which extends the plain `NauttPricingAdapterError` so every existing call site keeps its current behavior. Every other status, an unparseable body, or a transport failure stays the plain adapter error — never widen the allowlist without new approved research, and never log the request body, amount, or API key.
- Parse `data.currency.symbol` from the onramp creation `201` and `GET /orders/{uuid}` with the same strictness as every other order field: absent `currency` (documented for crypto-kind orders) parses cleanly with `currencySymbol` omitted, but a present, malformed `currency` still fails parsing.
...
- A `400`/`422` response with a parseable body and a `code` in the closed documented allowlist (`order.quote_not_found`, `order.quote_expired`, `order.exchange_not_configured`, `order.payment_method_not_available`, `orders.deposit_bank_account_not_found`, `order.deposit_fields_required`, `order.deposit_fields_validation_failed`) is a typed, code-carrying refusal — a deterministic terminal outcome, classified from the response already received, never a second dispatch. Every other 400/422 (undocumented or absent code, unreadable body) stays indeterminate, exactly like network failure, timeout, 5xx, or an invalid 201. On a refusal, the owner service discards the provider-order row while the quote's claim is never reset — the quote stays permanently unclaimable, never widening the `provider_order.creation_state` closed database check.
...
- Reconciliation writes never clear a stored `pixCopyPaste`/`pixQrcodeUrl`: omit those keys from the write when the authoritative read carries no value, and only overwrite them when the read carries one.

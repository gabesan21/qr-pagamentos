...
import { createStandalonePaymentStatusService } from "./standalone-payment-status";
...
const bearer = createHmac("sha256", key).update(`standalone-checkout-capability:v1:${attempt.id}:${expiresAt.toISOString()}:${attempt.capabilityNonce}`).digest("base64url");
attempt.capabilityVerifier = createHash("sha256").update(bearer).digest("hex");

describe("standalone payment status", () => {

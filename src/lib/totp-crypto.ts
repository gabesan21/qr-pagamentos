...
export function encrypt(plaintext: string, key: Buffer): string {
  if (key.length !== KEY_LENGTH) {
    throw new TotpCryptoError("Encryption key must be 32 bytes");
  }
...
function parseComposite(composite: string): ParsedComposite {
  ...
  if (parts.length !== 3) {
    throw new TotpCryptoError("Invalid ciphertext format");
  }
  const iv = Buffer.from(parts[0], "base64url");
  const ciphertext = Buffer.from(parts[1], "base64url");
  const authTag = Buffer.from(parts[2], "base64url");
  if (iv.length !== IV_LENGTH) {
    throw new TotpCryptoError("Invalid initialization vector");
  }
...
function decryptWithKey(parsed: ParsedComposite, key: Buffer): string {
  ...
  try {
    ...
    throw new TotpCryptoError("Decryption failed");
  }
}
...
export function loadEncryptionKey(): Buffer {
  ...
  if (!raw) {
    throw new TotpCryptoError("TOTP_ENCRYPTION_KEY is not set");
  }
  const key = Buffer.from(raw, "base64url");
  if (key.length !== KEY_LENGTH) {
    throw new TotpCryptoError("TOTP_ENCRYPTION_KEY must decode to 32 bytes");
  }
  return key;
}
...
export function loadPreviousEncryptionKey(): Buffer | undefined {
  ...
  const key = Buffer.from(raw, "base64url");
  if (key.length !== KEY_LENGTH) {
    throw new TotpCryptoError("TOTP_ENCRYPTION_KEY_PREVIOUS must decode to 32 bytes");
  }
  return key;
}

...
type CheckoutV2PaymentState = "RESERVED" | "CREATING" | PaymentLinkOrderState;
type Payment = Readonly<{ state: CheckoutV2PaymentState; pixCopyPaste?: string; pixQrCodeUrl?: string }>;
...
function paymentCopy(dictionary: Dictionary, state: CheckoutV2PaymentState): string {
  if (state === "RESERVED" || state === "CREATING") return dictionary.checkoutStateCreated;
...
export function paymentFromResponse(value: unknown): Payment | null {
  ...
  if (typeof payment.state !== "string" || !PAYMENT_STATES.has(payment.state as CheckoutV2PaymentState)) return null;
...
export function PublicCheckoutV2Form({ dictionary, identifier, policy }: Readonly<{ dictionary: Dictionary; identifier: string; policy: CheckoutDataPolicy }>) {

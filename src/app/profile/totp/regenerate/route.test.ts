...
vi.mock("@/auth/totp-store", () => ({ getTotpService: () => ({ regenerateRecoveryCodes }) }));

import { POST } from "./route";

const sameOrigin = { origin: "http://local", host: "local" };
const request = () => new Request("http://local/profile/totp/regenerate", { method: "POST", headers: sameOrigin });

describe("profile TOTP regenerate route", () => {
  it("rejects cross-origin posts", async () => {
    const response = await POST(new Request("http://local/profile/totp/regenerate", { method: "POST", headers: { host: "local" } }));
    expect(response.status).toBe(403);
  });

  it("returns new recovery codes on success", async () => {
    requireOwner.mockResolvedValueOnce({ id: "user-1", username: "owner" });
    regenerateRecoveryCodes.mockResolvedValueOnce(["a", "b"]);
  ...
  it("returns opaque unavailable on service error", async () => {
    requireOwner.mockResolvedValueOnce({ id: "user-1", username: "owner" });
    regenerateRecoveryCodes.mockRejectedValueOnce(new Error("unavailable"));
    const response = await POST(request());

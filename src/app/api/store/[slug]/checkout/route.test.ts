...
vi.mock("@/checkout/standalone-checkout", () => ({ getStandaloneCheckoutService: () => ({ checkout }) }));
vi.mock("@/security/public-rate-limit", () => ({
  allowPublicPaymentLinkRequest: allowRateLimit,
  publicPaymentLinkRateLimitSurface: { standaloneCheckout: "standalone-checkout-submit" },
...
const body = { idempotencyKey: "retry-key-with-enough-entropy", amount: "12.5", customer: { name: null, email: null, cpf: null, address: null } };
...
describe("POST /api/store/[slug]/checkout", () => {
  ...
  it("is sessionless, dynamic and maps only the redacted accepted capability", async () => {
    ...
    const response = await POST(new Request(`https://example.test/api/store/${slug}/checkout`, { method: "POST", headers: { "content-type": "application/json", cookie: "qr_session=ignored" }, body: JSON.stringify(body) }), context);

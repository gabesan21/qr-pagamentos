...
vi.mock("@/checkout/standalone-payment-status", () => ({ getStandalonePaymentStatusService: () => ({ read }) }));
vi.mock("@/security/public-rate-limit", () => ({
  allowPublicPaymentLinkRequest: allowRateLimit,
  publicPaymentLinkRateLimitSurface: { standaloneStatus: "standalone-payment-status-poll" },
  publicRateLimitResponse: () => new Response(null, { status: 429, headers: { "Cache-Control": "no-store" } }),
}));

import { dynamic, POST } from "./route";

describe("POST /api/store/[slug]/checkout/status", () => {
  beforeEach(() => {
    vi.clearAllMocks();

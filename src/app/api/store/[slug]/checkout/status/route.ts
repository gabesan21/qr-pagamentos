import { getStandalonePaymentStatusService } from "@/checkout/standalone-payment-status";
import {
...
export async function POST(request: Request) {
  return withServerRequestLog(request.headers.get("x-request-id"), { method: "POST", route: serverRequestRoutes.standaloneCheckoutStatus }, async () => {
    if (!allowPublicPaymentLinkRequest(request, publicPaymentLinkRateLimitSurface.standaloneStatus)) {
    ...
    const statusCapability = body && typeof body === "object" && !Array.isArray(body)
      ...
      : null;
    const payment = await getStandalonePaymentStatusService().read(statusCapability);

...
load_install_env() {
  ...
  while IFS= read -r line || [[ -n $line ]]; do
    ...
    case "$key" in
      APP_PORT|POSTGRES_ADMIN_PASSWORD|MIGRATOR_PASSWORD|RUNTIME_PASSWORD|INITIAL_ADMIN_USERNAME|INITIAL_ADMIN_EMAIL|NAUTT_ENCRYPTION_KEY|NAUTT_ENCRYPTION_KEY_PREVIOUS|TOTP_ENCRYPTION_KEY|TOTP_ENCRYPTION_KEY_PREVIOUS|NAUTT_WEBHOOK_CALLBACK_URL|NAUTT_API_BASE_URL|SMTP_HOST|SMTP_PORT|SMTP_USER|SMTP_PASSWORD|SMTP_FROM|SMTP_TLS_MODE|PUBLIC_ORIGIN|ALLOW_LOOPBACK_OPERATOR_ORIGINS)
  ...
  # Encryption keys are required at runtime but may be generated by the installer when absent/empty.
  for key in NAUTT_ENCRYPTION_KEY TOTP_ENCRYPTION_KEY; do
    : "${!key:=}"
  done
  # Rotation-window previous keys are optional and never generated: an
  # absent/empty value means no rotation window is open (today's behavior).
  for key in NAUTT_ENCRYPTION_KEY_PREVIOUS TOTP_ENCRYPTION_KEY_PREVIOUS; do
    : "${!key:=}"
  done
...
write_totp_encryption_key_source() {
  local source_dir=$ROOT_DIR/.install-secrets
  run mkdir -p -m 0700 "$source_dir"
  if [[ -n ${TOTP_ENCRYPTION_KEY:-} ]]; then
    if "$DRY_RUN"; then
      printf 'DRY-RUN validate and stage provided totp_encryption_key\n'
    else
      if ! run_node_helper -e 'const b = Buffer.from(process.argv[1], "base64url"); process.exit(b.length === 32 ? 0 : 1)' "$TOTP_ENCRYPTION_KEY" >/dev/null; then
        die 'TOTP_ENCRYPTION_KEY must decode to exactly 32 bytes using base64url'
      fi
      (umask 077; printf '%s' "$TOTP_ENCRYPTION_KEY" > "$source_dir/totp_encryption_key")
      chmod 0600 "$source_dir/totp_encryption_key"
    fi
  else
    if "$DRY_RUN"; then
      printf 'DRY-RUN generate totp_encryption_key and warn operator to back it up\n'
    else
      (umask 077; run_node_helper -e 'const crypto = require("node:crypto"); process.stdout.write(crypto.randomBytes(32).toString("base64url"))' > "$source_dir/totp_encryption_key")
      chmod 0600 "$source_dir/totp_encryption_key"
      printf 'WARN: a new TOTP_ENCRYPTION_KEY was generated; back up %s/totp_encryption_key outside this host. Losing it makes every stored TOTP secret irrecoverable.\n' "$source_dir" >&2
    fi
  fi
  TOTP_ENCRYPTION_KEY_FILE=$source_dir/totp_encryption_key
}

# Rotation-window previous keys are optional and never generated: validate and
# stage a provided value; otherwise write an empty file (the established
# optional-secret convention — see initial_admin_email) so the rewrap one-shot
# and the app container can tell "not configured" from "malformed" without an
# absent path. Never rotates, regenerates, or overwrites an existing value.
write_previous_encryption_key_source() {
  local variable=$1 target=$2 label=$3
  local source_dir=$ROOT_DIR/.install-secrets
  run mkdir -p -m 0700 "$source_dir"
  if [[ -n ${!variable:-} ]]; then
    if "$DRY_RUN"; then
      printf 'DRY-RUN validate and stage provided %s\n' "$target"
    else
      if ! run_node_helper -e 'const b = Buffer.from(process.argv[1], "base64url"); process.exit(b.length === 32 ? 0 : 1)' "${!variable}" >/dev/null; then
        die "$label must decode to exactly 32 bytes using base64url"
      fi
      (umask 077; printf '%s' "${!variable}" > "$source_dir/$target")
      chmod 0600 "$source_dir/$target"
    fi
    ...
    if "$DRY_RUN"; then
      printf 'DRY-RUN write empty %s (no rotation window configured)\n' "$target"
    else
      (umask 077; : > "$source_dir/$target")
      chmod 0600 "$source_dir/$target"
    fi
  fi
  printf -v "${variable}_FILE" '%s' "$source_dir/$target"
}
...
stage_secrets() {
  ...
  for entry in \
    ...
    "NAUTT_ENCRYPTION_KEY_PREVIOUS_FILE:nautt_encryption_key_previous" \
    "TOTP_ENCRYPTION_KEY_FILE:totp_encryption_key" \
    "TOTP_ENCRYPTION_KEY_PREVIOUS_FILE:totp_encryption_key_previous" \
...
if ! "$DRY_RUN" && docker volume inspect "${PROJECT}_postgres-data" >/dev/null 2>&1; then
  ...
  TOTP_ENCRYPTION_KEY_FILE=$ROOT_DIR/.install-secrets/totp_encryption_key
  ...
  # Ensure new source files exist when upgrading a retained deployment.
  # NAUTT/TOTP_ENCRYPTION_KEY_PREVIOUS stay optional here too: an unset
  # variable backfills an empty file (no rotation window), never a generated
  # or rotated value.
  for entry in \
    "TOTP_ENCRYPTION_KEY:totp_encryption_key" \
    "NAUTT_ENCRYPTION_KEY_PREVIOUS:nautt_encryption_key_previous" \
  ...
  write_nautt_encryption_key_source
  write_totp_encryption_key_source
  write_previous_encryption_key_source NAUTT_ENCRYPTION_KEY_PREVIOUS nautt_encryption_key_previous NAUTT_ENCRYPTION_KEY_PREVIOUS
  write_previous_encryption_key_source TOTP_ENCRYPTION_KEY_PREVIOUS totp_encryption_key_previous TOTP_ENCRYPTION_KEY_PREVIOUS
  write_identity_sources

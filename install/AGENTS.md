...
- With retained data, validate every PostgreSQL, Nautt, and TOTP encryption source/staged/supplied value and authenticate all three database roles before secret or deployment mutation. Never rotate, regenerate, overwrite, print, hash-log, or path-log continuity material. The sole exception is the explicit operator-invoked `docker compose --profile rotate run --rm rotate-encryption-keys` rewrap one-shot ([production runbook](../docs/production-runbook.md)); install and update never invoke it and never stage a `*_ENCRYPTION_KEY_PREVIOUS` value on their own.

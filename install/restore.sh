...
docker exec "$candidate_db" sh -eu -c 'PGPASSWORD=$(cat /run/admin); export PGPASSWORD; createdb -U postgres -p 5433 qr_pagamentos; psql -U postgres -p 5433 -d qr_pagamentos -f /run/roles.sql; pg_restore -U postgres -p 5433 -d qr_pagamentos --no-owner --no-acl /run/database.dump' \
...
restore_managed_pair() {
  ...
  docker run --rm --network "${PROJECT}_database" --read-only --tmpfs /tmp \
    ...
    "$POSTGRES_IMAGE" sh -eu -c 'PGPASSWORD=$(cat /run/admin); export PGPASSWORD; pg_restore -h db -p 5433 -U postgres -d qr_pagamentos --no-owner --no-acl --clean --if-exists /run/database.dump' >/dev/null
  docker run --rm --network none --read-only --tmpfs /tmp --user 1000:1000 \

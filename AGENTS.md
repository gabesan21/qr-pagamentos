...
- `src/orders/order-v2.ts` owns the Commerce V2 order service: owner-scoped `AD_HOC` creation (stateless under this contract — no `state`, link, attempt, capability, or provider dispatch) behind `POST /orders-v2`, the server-only `LINK` creation seam with immutable exact-decimal monetary snapshot lines (BigInt micro-units, never JS `Number`), and the settlement-map V1 reconciliation CAS plus the atomic `payment_link_v2_single_use_settlement` claim rebound to V2 identities. The source vocabulary (`LINK`|`AD_HOC`) with per-source link/state coherence, the canonical decimal grammar, and active-dependency enforcement are service-fenced, never database constructs. `POST /orders-v2` and `POST /orders-v2/[id]` are the only V2 order mutation routes — origin-guard first, re-authorized cookie merchant, empty `401`/`403`, opaque `/orders?orders-v2=…` redirects, two literal server-request-log entries; never add a public/sessionless V2 checkout here (9.3.1 owns it).
...
- `src/orders/order-v2-view.ts` owns the only server-only read projection through which V2 order data leaves persistence: re-authorized owner and administrator list/detail reads exposing source, the immutable monetary snapshot, the policy-exact `CustomerSnapshotV1` under the fail-closed guard, `state`, and the current local outcome as separate fields, with one opaque "unavailable" outcome for cross-owner, malformed, and missing identities. V1 `src/orders/order-view.ts` keeps reading only V1 data.
- `src/auth/payment-link-v2-view.ts` owns the read-time derived lifecycle projection for the merchant V2 payment-link directory: `inactive` > `expired` > `paid` > `active` precedence computed at read time (never a stored column; `paid` is the SINGLE_USE settlement claim or at least one CONFIRMED LINK order), the foundation `DirectoryAdapter` over the keyset `(created_at, id)` window with `pageSize + 1` bounded reads and owner scope re-checked per read, and the `/links/v2/[id]` owner detail read with one opaque "unavailable" outcome for cross-owner, malformed, and missing identities. Never expose V1 data, provider data, verifiers, or a mutation through it, and never persist a derived state. See [`pop/specs/catalog-and-payment-links.md`](pop/specs/catalog-and-payment-links.md).

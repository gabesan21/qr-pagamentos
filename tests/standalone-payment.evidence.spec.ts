...
function seedAttempt(options: { state: string }) {
  ...
  const sql = [
    ...
    `INSERT INTO app.provider_order (owner_id, quote_uuid, provider_order_uuid, creation_state, status, pix_copy_paste, pix_qrcode_url, order_v2_id, created_at, updated_at)
     SELECT u.id, '${quoteUuid}', '${randomUUID()}', 'CREATED', 'PENDING', 'pix-copy-paste-evidence-code', '${baseUrl}/file.svg', '${orderId}', '${at}', '${at}'
     FROM app."user" u WHERE u.username = '${merchantUsername}'`,
  ].join(";\n") + ";\n";

...
- A delivery claim must serialize attempt numbering, terminal replay, live-lease exclusion, and expired-lease recovery; finalization must match the current processing attempt so an expired worker is fenced. Never hold its transaction across provider I/O or finalize before authoritative reconciliation succeeds.

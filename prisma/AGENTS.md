...
- `app.standalone_checkout_attempt` (9.2.1) mirrors the attempt columns bound to `order_v2_id` (unique, composite owner-bound restrictive FK) with replay uniqueness keyed by `(owner_id, retry_key_verifier)` and the `(owner_id, capability_expires_at)` index; runtime holds exactly `SELECT`/`INSERT`/`UPDATE` (attempts are stateful, not append-only, so no relation-guarded re-pin) — never add a link column, backfill, or grant `DELETE`.

...
assert(
  envExample === "MIGRATION_DATABASE_URL=<postgresql-migrator-url>\nDATABASE_URL=<postgresql-runtime-url>\nNAUTT_ENCRYPTION_KEY=<32-byte-base64url-key>\nTOTP_ENCRYPTION_KEY=<32-byte-base64url-key>\n# Optional rotation-window keys: set only while rotating NAUTT_ENCRYPTION_KEY\n# or TOTP_ENCRYPTION_KEY. Absent reproduces today's behavior exactly; present\n# must decode to the same 32-byte base64url shape as the current key.\n# NAUTT_ENCRYPTION_KEY_PREVIOUS=<32-byte-base64url-key>\n# TOTP_ENCRYPTION_KEY_PREVIOUS=<32-byte-base64url-key>\nNAUTT_WEBHOOK_CALLBACK_URL=https://payments.example.com/api/nautt/webhooks\nNAUTT_API_BASE_URL=<optional-https-override-default-https://api.nauttfinance.com/api/v2>\n\n# Required self-hosted SMTP configuration for password-reset email delivery.\nSMTP_HOST=smtp.example.com\nSMTP_PORT=587\nSMTP_USER=replace-with-smtp-username\nSMTP_PASSWORD=replace-with-smtp-password\nSMTP_FROM=noreply@example.com\nSMTP_TLS_MODE=starttls\n\n# Required canonical public origin of this deployment (absolute HTTPS, no credentials or fragment).\nPUBLIC_ORIGIN=https://payments.example.com\n\n# In a production build (NODE_ENV=production), a loopback HTTP PUBLIC_ORIGIN or\n# NAUTT_WEBHOOK_CALLBACK_URL is refused unless ALLOW_LOOPBACK_OPERATOR_ORIGINS\n# is set to exactly \"1\". Never set this in a real deployment; the installer\n# derives and forwards it automatically when the operator's own origin is loopback.\n# ALLOW_LOOPBACK_OPERATOR_ORIGINS=1\n",

...
| `pop_validate.py` | Valida limites: 144/600 chars nos índices, ≤150 linhas por nota (`raw/` de pesquisa isento), frontmatter dos cards, `stage:` coerente, anotações `pop-hash` de citação de código (fail-closed — regra 9 do DOX, [[_templates/DOX]]); avisos: worktrees órfãs e wikilinks quebrados. Exit 1 se houver violação. |

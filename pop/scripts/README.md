...
| `pop_install_included.py <dir>` | Instala/**atualiza** o harness declarado em `_templates/included-manifest.json`: espelha o conjunto gerido no alvo, **poda** o que saiu da origem e carimba nele o `content_sha` do harness de origem. `--check-fresh <dir>` recomputa e falha fechado se o alvo ficou atrás (o `pop_validate` acusa como violação); `--sha` imprime a versão da origem; `--check` só confere se está instalado; `--audit-manifest` verifica o fechamento da fonte única. |

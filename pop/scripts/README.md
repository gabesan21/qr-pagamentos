...
| Script | Faz |
|--------|-----|
| `pop_status.py` | Panorama: tasks por estágio/projeto, gates pendentes (liberação em 001, 003, 005 crítica, merge), bloqueadas, paradas >14 dias, claims ativos, alerta de WIP > 3. `--project <proj>[/<repo>]` filtra. |
...
| `pop_task.py <proj> <id>` | Scaffolding: card em `001_initial_task` a partir de `_templates/TASK.md` + `subtasks/` vazia, com o bloco de frontmatter da origem do id (roadmap ou `M-`). `--title "..."` define o título. Repo de multi-repo: `<proj>/<repo>`. |
| `pop_worktree.py route\|add\|remove <task>` | Consome a rota Git da task; escopo local recusa worktree e fica em `main`; yolo externo cria branch de task a partir da branch corrente de trabalho do repo e o PR final é só a pedido do humano. Nos demais casos cria/remove `worktrees/<id>` normalmente. |
...
| `pop_delivery.py integrate <task>` | Integra idempotentemente `task/<id>` na branch corrente de trabalho; `scope-pr [--base B]` abre/reusa o PR dela para `main` (ou `--base`), somente a pedido do humano, sem merge. |
...
`poplib.py` é o módulo compartilhado: raiz do escopo corrente (a busca para no marcador `pop/.unirepo-harness.json` — harness instalado é um mundo completo), descoberta dos escopos de projeto hospedados, rótulo de projeto e parser próprio de frontmatter (sem PyYAML).

Exemplo:

```
python3 pop/scripts/pop_task.py meu-projeto 1.1.1-user-table-creation --title "Tabela de usuários"
python3 pop/scripts/pop_task.py meu-projeto M-1.1-ajusta-contrato --title "Ajusta contrato"

...
- Restore has no partial or force mode. It rejects followed artifact symlinks and requires archive entries to be owned by UID/GID 1000 with private `0700` directories and `0600` regular objects. It validates exact revision/project/volume identity, rehearses the pair and exact-release health in a deterministic disposable DB/media/network/secret inventory, proves that inventory absent, then restores and re-verifies database metadata plus media inventory on the same managed volume IDs.

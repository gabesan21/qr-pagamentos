...
| M-4 | Test baseline determinism | `M-4.1-repair-test-baseline-determinism` repaired the three carried test failures and excluded task worktrees from `pnpm test`. `M-4.2-exclude-worktrees-from-tooling` extends the exclusion to the remaining tooling (typecheck/lint). · size: S · yolo: yes | in progress |

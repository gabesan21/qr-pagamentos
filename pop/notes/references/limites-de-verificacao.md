...
- **Bare `pnpm <script>` aborts on a Node/pnpm mismatch worktree** — host Node 26.10.0 vs pinned 26.4.0 makes pnpm's deps-status check try a non-interactive `install` and fail closed with `ERR_PNPM_ABORTED_REMOVE_MODULES_DIR_NO_TTY` before the script runs (seen in 13.4.2). Fall back to the direct binary (`./node_modules/.bin/tsc`, `./node_modules/.bin/eslint .`, `./node_modules/.bin/vitest run`, `./node_modules/.bin/next build`, or the underlying `node scripts/*.mjs`/`node pop/scripts/*.mjs`) instead of forcing `CI=true pnpm run <script>` — the latter still works but performs a real install pass against a `node_modules` that may be a shared symlink with the main checkout, which is an avoidable side effect the fallback route does not have.

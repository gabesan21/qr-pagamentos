...
- **Applied directly, outside 004_processing:** these edits were made directly on `main` at the user's explicit instruction, not through task [[1.1.3-containerize-self-hosted-runtime]]'s worktree/branch. This superseded the installer contract that task's plan and Round 4 verification (`kanban/005_verifying/1.1.3-containerize-self-hosted-runtime/1.1.3-containerize-self-hosted-runtime.verify.md`) had assumed and passed. The decision has been applied and delivery confirmed in [[memory/1.1.3-containerize-self-hosted-runtime|1.1.3-containerize-self-hosted-runtime]]; criterion 9 is re-verified against the current `install/install.sh`/`install/uninstall.sh`.

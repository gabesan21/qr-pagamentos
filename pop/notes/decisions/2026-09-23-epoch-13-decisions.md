...
5. **Currency-pair validation is planned from the repository's own Nautt documentation** (`researches/nautt-finance/raw/**`, `nautt-finance.md`), by user command of 2026-09-23 superseding the earlier research block: points those documents do not cover are declared gaps in the plan (the `/exchange-currencies` prompt stays in `RESEARCHES.md` as a standing gap), and no external or invented source is consulted.

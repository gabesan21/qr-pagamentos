...
- **Merge é do humano:** o agente nunca merga PR de task por conta própria — só quando comandado na rodada de merge. O humano pode comandar merge a qualquer momento, inclusive **fora de 006** e para testar o entregável (ver "Comando explícito do humano vence o fluxo"). Exceção: task yolo, cujo PR mira `develop` e é mergeado pelo crítico; o PR final `develop` → branch de PR continua sendo do humano (seção Yolo mode).

...
- **Nada aqui autoriza subir.** Versão do harness, panorama de outros projetos e índices de agregação são responsabilidade de quem instalou este harness. Escopo instalado responde sobre si pelo `pop/.unirepo-harness.json` e para aí; comparar com a origem não é trabalho dele.
...
- **001:** fica com o agente principal; em yolo externo, a integração na branch de trabalho corrente também é mecânica dele. Escopo local opera direto em `main`.
...
**Ato 2 — integração e PR.** Escopo local já está em `main`, sem branch/worktree/PR da task. Escopo externo **não-yolo**: abra o PR da task, marque `pr:` e `awaiting_merge: true` e aguarde o merge humano. Escopo externo **yolo**: integre mecanicamente na branch de trabalho corrente, sem PR por task. Todo PR carrega a seção **Verificação humana**: os critérios `verify: user` acumulados (incluindo os reclassificados por `ambiente`), cada um com o passo manual e o pass esperado; sem PR (escopo local), a checklist entra na rodada/open question de aprovação final — o humano é o verificador de última instância desses itens.
...
5. Se esta foi a última task de escopo yolo externo, **sugira ao humano** o PR da branch de trabalho corrente para `main` — o agente nunca abre PR por conta própria; somente a pedido explícito do usuário ele o cria (`pop_delivery.py scope-pr`). O merge é sempre humano. Sem Git, crie a rodada de aprovação final.
...
- **Merge humano no fim do escopo marcado** — task avulsa, phase/epoch ou modification: fora do escopo local, o orquestrador, não o juiz, integra cada task na branch de trabalho corrente (a mesma da qual a worktree partiu), sem PR por task. Quando a última task do escopo fecha o `005_closing`, o orquestrador **sugere** ao humano o PR da branch de trabalho para `main`, com resumo/testes/criticals e a checklist de verificação humana (critérios `verify: user` do escopo); o PR só é aberto a pedido explícito do usuário e o merge é sempre humano. Em escopo local tudo permanece em `main`, sem branch/worktree/PR da task ou do escopo.

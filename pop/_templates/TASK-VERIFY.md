...
> O Judge Dredd ([[.agents/skills/judge-dredd/SKILL|judge-dredd]]) nasce em sessão limpa com seu perfil nativo fixo e decide `differential|full`: `full` em critical ou retorno por `premissa`; depois de `lacuna` ou falha de execução, o diferencial cobre o **delta**.

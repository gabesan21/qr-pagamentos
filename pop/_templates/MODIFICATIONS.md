...
> Linhas concluídas permanecem com status `concluída` (como as epochs no ROADMAP); são as linhas de **task** que saem dos arquivos de modification após a memory válida (regra 17 do [[AGENTS|AGENTS]]). Ids `M-<n>` nunca são reutilizados.

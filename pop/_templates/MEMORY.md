...
authorization: <D-AAAAMMDD-<slug>: rota sem kanban (plan mode) escolhida na triagem · F-AAAAMMDD-<slug>: triagem de fix direto (regra 13)>
---
...
> Trabalho pela rota sem kanban (plan mode do coding agent) usa `task: D-AAAAMMDD-<slug>`; **fix direto** aprovado na triagem da regra 13 usa `task: F-AAAAMMDD-<slug>`. Ambos preenchem `authorization` e não possuem card nem linha de roadmap/modifications — mas têm ledger e entradas como qualquer outra.

...
| Task | Description | Status |
|------|-------------|--------|
| [[12.2.1-rebuild-template-tokens-and-typography]] | Port the exact six palettes, spacing, radii, elevations, motion, and typography through DTCG primitive/semantic/component layers in `src/design-system/**` and generated CSS. · size: L | 001_initial_task |
...
| Task | Description | Status |
|------|-------------|--------|
| `12.5.1-remodel-merchant-dashboard` | Match the template dashboard in `src/app/(merchant)/page.tsx` and `dashboard.tsx` while preserving existing analytics, period, currency, and storefront-link behavior. · size: L | não iniciada |
| `12.5.2-remodel-merchant-order-management` | Match owner order directories, V1/V2 details, comments, local outcomes, and unavailable states under `src/app/(merchant)/orders/**` and shared order views. · size: L | não iniciada |
...
| Task | Description | Status |
|------|-------------|--------|
| `12.6.1-remodel-public-payment-link-checkout` | Match the template V1/V2 buyer flow, policy-specific forms, QR/copy, polling, retry, privacy, paid, unavailable, and terminal states under `src/app/pay/[identifier]/**`. · size: L | não iniciada |
...
| `12.7.2-prove-full-frontend-template-parity` | Verify every route/state in `pt-BR` and `en` across six themes and 320/375/768/1440 widths with visual, keyboard, WCAG 2.2 AA, contract, and `pnpm check` evidence. · size: L | não iniciada |
...
- Phases 12.4 and 12.5 may run independently after Phase 12.3, with at most three parallel tasks whose route, dictionary, test, and evidence write sets are disjoint.
